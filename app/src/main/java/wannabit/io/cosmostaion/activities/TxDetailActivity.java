...
public class TxDetailActivity extends BaseActivity implements View.OnClickListener, Dialog_MoreWait.OnTxWaitListener {
    ...
    public void onClick(View v) {
        if (v.equals(mDismissBtn)) {
            ...
            if (getBaseChain().equals(BNB_MAIN.INSTANCE)) {
                if ((getBalance(BNB_MAIN.INSTANCE.getMainDenom())).compareTo(new BigDecimal(FEE_BNB_SEND)) < 0) {
        ...
        private void onBindTransfer(RecyclerView.ViewHolder viewHolder, int position) {
            ...
            if (getBaseChain().equals(BNB_MAIN.INSTANCE)) {
                ...
                if (msg.type.equals(COSMOS_MSG_TYPE_TRANSFER2) || msg.type.equals(CERTIK_MSG_TYPE_TRANSFER)) {
                    ...
                    try {
                        holder.itemFromAddress.setText(WalletUtils.INSTANCE.convertAddressOkexToEth(msg.value.from_address));
                        holder.itemToAddress.setText(WalletUtils.INSTANCE.convertAddressOkexToEth(msg.value.to_address));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    if (getAccount().address.equals(msg.value.from_address)) {
                    ...
                    try {
                        holder.itemFromAddress.setText(WalletUtils.INSTANCE.convertAddressOkexToEth(msg.value.from));
                        holder.itemToAddress.setText(WalletUtils.INSTANCE.convertAddressOkexToEth(msg.value.transfers.get(0).to));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
