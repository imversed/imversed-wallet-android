...
public class DAppsList5Activity extends BaseActivity implements TaskListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tabItemText3.setTextColor(WDp.getTabColor(this, getBaseChain()));
    ...
    public void onCheckStartSwap(String inputCoinDenom, String outCoinDenom, QueryOuterClass.PoolResponse swapPool) {
        ...
        BigDecimal txFee = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_KAVA_SWAP, 0);
    ...
    public void onCheckStartJoinPool(QueryOuterClass.PoolResponse myPool) {
        ...
        BigDecimal feeAmount = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_KAVA_JOIN_POOL, 0);
    ...
    public void onCheckStartExitPool(QueryOuterClass.PoolResponse myPool, QueryOuterClass.DepositResponse myDeposit) {
        ...
        BigDecimal feeAmount = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_KAVA_EXIT_POOL, 0);
    ...
    public void onFetchData() {
        ...
        new KavaSwapParamsGrpcTask(getBaseApplication(), this, getBaseChain()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        new KavaSwapPoolsGrpcTask(getBaseApplication(), this, getBaseChain()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        new KavaSwapDepositGrpcTask(getBaseApplication(), this, getBaseChain(), getAccount()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        new KavaCdpParamGrpcTask(getBaseApplication(), this, getBaseChain()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        new KavaHardParamGrpcTask(getBaseApplication(), this, getBaseChain()).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }
            ...
            for (QueryOuterClass.PoolResponse pool : mSwapPoolList) {
                for (CoinOuterClass.Coin coin : pool.getCoinsList()) {
                    if (!mAllDenoms.contains(coin.getDenom())) {
                        ...
                        WUtil.onSortingDenom(mAllDenoms, getBaseChain());
                    }
                }
