...
public class GravityListActivity extends BaseActivity implements TaskListener {
    ...
    public void onStartSwap(String inputCoinDenom, String outCoinDenom, Liquidity.Pool pool) {
        ...
        BigDecimal txFee = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_GDEX_SWAP);
    ...
    public void onCheckStartDepositPool(long poolId) {
        ...
        BigDecimal feeAmount = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_GDEX_DEPOSIT);
        BigDecimal coin0Available = getBalance(coinDenom0);
        BigDecimal coin1Available = getBalance(coinDenom1);

        if (coinDenom1.equalsIgnoreCase(COSMOS_MAIN.INSTANCE.getMainDenom())) {
    ...
    public void onCheckStartWithdrawPool(long poolId) {
        ...
        BigDecimal feeAmount = getBaseChain().getGasFeeEstimateCalculator().calc(getBaseChain(), CONST_PW_TX_GDEX_WITHDRAW);
    ...
    public void onTaskResponse(TaskResult result) {
        ...
        if (result.taskType == TASK_GRPC_FETCH_GRAVITY_POOL_LIST) {
            ...
            for (Liquidity.Pool pool : mPoolList) {
                ...
                if (!getBalance(pool.getPoolCoinDenom()).equals(BigDecimal.ZERO)) {
    ...
    public BigDecimal getGdexMyShareAmount(Liquidity.Pool pool, String denom) {
        BigDecimal result = BigDecimal.ZERO;
        BigDecimal myShare = getBalance(pool.getPoolCoinDenom());
        BigDecimal totalShare = BigDecimal.ZERO;
        for (Coin coin : getBaseDao().mGDexPoolTokens) {
            if (coin.denom.equalsIgnoreCase(pool.getPoolCoinDenom())) {
                totalShare = new BigDecimal(coin.amount);
